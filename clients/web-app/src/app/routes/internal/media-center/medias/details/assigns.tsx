...
import type { Route } from "./+types/assigns";
import {
  deleteMediaAssign,
  getMediaAssigns,
} from "@/api/services/medias.service";
...
import { AddMediaAssignModal } from "@/components/modals/add-media-assign-modal";
  ...
  return (
    <div>
      <DataTable
        ...
        addButton={<AddMediaAssignModal mediaId={mediaId} />}
        fetch={({ page }) => getMediaAssigns(mediaId, { page })}
        ...
        columns={[
...

          {
            ...
            content: {
              render(value, refetch) {
                return value.relations?.title ? (
                  <Link
                    to={`/content-center/titles/${value.relations.title.id}`}
                  >
                    {value.relations.title.name}
                  </Link>
                ) : (
                  "N/A"
                );
        ...
        // footerChildren={<Button size="sm">Add assign</Button>}
      />
    </div>
  );
}
